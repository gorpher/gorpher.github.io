...
```
### 使用URLConnection发送post 和 get 请求
```java
    //post
...
# apache 接口  httpclient  现在可能已经被apache废弃了  好像用的httpclient4  暂时不会用  
### 使用HttpClient发送post和get请求  导入jar包  -->>httpclient-4.5.3.jar  httpcore-4.4.6.jar       commons-logging-1.2.jar
```java
